export default {
  lang: { zh: '中文', en: 'English' },

  sidebar: {
    managementSuite: 'Management Suite',
    addTask: 'Add Task',
    taskList: 'Task List',
    systemSettings: 'Settings',
    asrConfig: 'ASR Config',
    llmConfig: 'LLM Config',
    materialConfig: 'Material Config',
    publishConfig: 'Publish Config',
    ttsConfig: 'TTS Config',
    proxyConfig: 'Proxy Config',
  },

  breadcrumb: {
    addTask: 'Add Task',
    addTaskNew: 'Add New Task',
    editTask: 'Edit Task',
    appName: 'VideoPrinterTurbo',
    taskList: 'Task List',
    settings: 'Settings',
    asr: 'ASR Config',
    llm: 'LLM Config',
    material: 'Material Config',
    publishConfig: 'Publish Config',
    ttsConfig: 'TTS Config',
    proxy: 'Proxy Config',
  },

  addTask: {
    downloadVideo: 'Download Video',
    audioToText: 'Audio to Text',
    llmRewrite: 'LLM Rewrite',
    outputToVoice: 'Output to Voice',
    outputToSubtitle: 'Output to Subtitle',
    bgm: 'Background Music',
    videoOverlay: 'Video Overlay',
    publish: 'Publish',

    videoInputMode: 'Video Source',
    localUpload: 'Local Upload',
    videoUrl: 'Video URL',
    videoUrlPlaceholder: 'Enter TikTok / Bilibili / YouTube video URL',
    checkLink: 'Check Link',
    downloadUseProxy: 'Use proxy server for download?',
    transcriptionMode: 'Transcription Method',
    fromSubtitle: 'Extract from Subtitles',
    fromAsr: 'Convert via ASR',
    subtitleLang: 'Subtitle Language',
    subtitleLangPlaceholder: 'Please select subtitle language',
    llmPrompt: 'LLM Prompt',
    llmPromptPlaceholder: 'Enter rewriting instructions, e.g.: Rewrite the following into a humorous short video script...',
    ttsService: 'TTS Service',
    voiceRole: 'Voice Role',
    testVoice: 'Test',
    speechVolume: 'Volume',
    speed: 'Speed',
    font: 'Font',
    position: 'Position',
    customPosition: 'Custom Position (% from top)',
    subtitleColor: 'Subtitle Color',
    strokeColor: 'Stroke Color',
    subtitleSize: 'Subtitle Size',
    bgmLibrary: 'BGM Library',
    uploadAudio: 'Click to upload local audio file',
    bgmUploadSuccess: 'Audio uploaded successfully',
    bgmUploadFailed: 'Audio upload failed, please try again',
    materialUploadSuccess: 'Video uploaded successfully',
    materialUploadFailed: 'Video upload failed, please try again',
    bgmPreview: 'Preview',
    bgmPreviewPause: 'Pause',
    bgmVolume: 'BGM Volume',
    videoSource: 'Video Source',
    concatMode: 'Concat Mode',
    transitionMode: 'Transition',
    videoAspect: 'Aspect Ratio',
    maxDuration: 'Max Clip Duration (s)',
    videoCount: 'Number of Videos',
    videoMaterialKeyword: 'Video Keywords (English, optional)',
    videoMaterialKeywordPlaceholder: 'Up to 5 English or numeric words; hyphens and apostrophes allowed',
    videoMaterialKeywordEnglishOnly: 'Video keywords may contain only English letters, numbers, hyphens, and apostrophes',
    videoMaterialKeywordWordLimit: 'Video keywords may contain at most 5 words',
    publishSettings: 'Publish Settings (JSON config)',
    uploadVideo: 'Click to upload local video files',
    uploadVideoHint: 'Supports MP4, MOV, AVI, etc. Multiple files allowed',
    uploadTaskVideo: 'Click to upload a local video file',
    uploadTaskVideoHint: 'Supports MP4, MKV, AVI, WMV, and FLV. One file only.',
    startTask: 'Start Task',
    usageGuide: 'Usage Guide',

    lightBgm: 'Light & Upbeat',
    calmBgm: 'Calm & Relaxing',
    randomBgm: 'Random Music',
    noBgm: 'No Music',
    localFile: 'Local Files',
    sequential: 'Sequential',
    randomConcat: 'Random (Recommended)',
    noTransition: 'No Transition',
    randomTransition: 'Random',
    fadein: 'Fade In',
    fadeout: 'Fade Out',
    fadeinout: 'Fade In/Out',
    slidein: 'Slide In',
    slideout: 'Slide Out',
    portrait: '9:16 (Portrait)',
    landscape: '16:9 (Landscape)',
    bottomCenter: 'Bottom Center (Recommended)',
    topCenter: 'Top Center',
    middle: 'Center',
    customSubtitlePos: 'Custom (70%, from top)',

    enterUrlFirst: 'Please enter the video URL first',
    featureInDev: 'Feature under development',
    checkLinkSuccess: 'Link is valid',
    checkLinkFailed: 'Link check failed: {msg} (code: {code})',
    checkLinkError: 'Check failed, please try again later',
    enterUrl: 'Please enter a video URL',
    uploadTaskVideoRequired: 'Please upload a local video file first',
    taskVideoUploadSuccess: 'Video uploaded successfully',
    taskVideoUploadFailed: 'Video upload failed, please try again',
    taskCreated: 'Task created',
    createFailed: 'Creation failed, please try again',
    taskUpdated: 'Task updated',
    updateFailed: 'Update failed, please try again',
    loadConfigFailed: 'Failed to load task config, please refresh and try again',
    loadTaskFailed: 'Failed to load task, please try again',
    testVoiceFailed: 'Voice preview failed, please try again',

    helpDownload: 'Choose either a video page URL and check the link, or upload a local video file directly. If a link cannot be downloaded, please submit an issue on GitHub.',
    helpTranscription: 'Two methods available: "Extract from Subtitles" works for YouTube and similar sites with auto-captions; "Convert via ASR" extracts audio directly (recommended for non-YouTube videos).',
    helpLlm: 'If you select "LLM Rewrite" and enter a prompt, the extracted text will be rewritten by the LLM and output again. Note: To use "LLM Rewrite," you must select the "Audio to Text" option.',
    helpVoice: '"Output to Speech" converts the processed text into audio using TTS and merges it into the new video. Note: To select "Output to Speech," you must also check "Audio to Text."',
    helpSubtitle: '"Output to Subtitles": The original video\'s audio remains unchanged; the processed text is simply converted into subtitles and added to the new video. Note: To select "Output to Subtitles," you must also check the "Audio to Text" option.',
    helpBgm: 'After selecting background music, it will be merged into the new video.',
    helpVideoOverlay: 'When enabled, short clips from Pexels and similar sites will be fetched and stitched together.',
    publishNotAvailable: 'This feature is not available yet',
    helpPublish: 'This feature is not yet available.',
  },

  taskList: {
    title: 'Task List',
    newTask: 'New Task',
    address: 'Address',
    localPath: 'Local Path',
    videoCategory: 'Video Category',
    networkVideo: 'Online Video',
    localVideo: 'Local Video',
    status: 'Status',
    operations: 'Operations',
    noPath: '— No path assigned —',
    viewLogs: 'View Logs',
    play: 'Play',
    retry: 'Retry',
    edit: 'Edit',
    showing: 'Showing {start}–{end} of {total} entries',
    statusComplete: 'Complete',
    statusFailed: 'Failed',
    statusInProgress: 'In Progress',
    statusPending: 'Pending',
    videoPlay: 'Play Video',
    noVideo: 'No video file',
    errorLog: 'Error Log',
    noError: 'No error info',
    retrySuccess: 'Task resubmitted',
    retryFailed: 'Retry failed',
    delete: 'Delete',
    deleteTitle: 'Confirm Delete',
    deleteConfirm: 'Are you sure you want to delete this task?',
    deleteSuccess: 'Deleted successfully',
    deleteFailed: 'Delete failed',
    confirm: 'Confirm',
    cancel: 'Cancel',
  },

  placeholder: {
    building: 'Feature Under Construction',
    comingSoon: 'Coming Soon',
  },

  asrConfig: {
    title: 'ASR Config',
    whisperTab: 'Whisper (Local ASR)',
    remoteWhisperTab: 'Whisper (Remote ASR)',
    remoteDeploymentType: 'Deployment Type',
    vllmDeployment: 'vLLM Whisper Deployment',
    whisperCppDeployment: 'Whisper.cpp Whisper Deployment',
    vllmUrl: 'Vllm URL',
    vllmModel: 'Vllm Model',
    whisperCppUrl: 'whisper.cpp URL',
    tencentTab: 'Tencent Cloud ASR',
    xunfeiTab: 'Xunfei ASR',
    openaiTab: 'OpenAI ASR',
    azureTab: 'Azure ASR',
    volcengineTab: 'Volcengine ASR',
    modelSelect: 'Model',
    placeholder: 'Enter value',
    reset: 'Reset',
    submit: 'Submit',
    submitSuccess: 'Configuration saved',
    submitFailed: 'Save failed, please try again',
    loadModelListFailed: 'Failed to load model list',
  },

  llmConfig: {
    title: 'LLM Config',
    usageGuide: 'Usage Guide',
    helpUsage: 'Currently only one LLM configuration is supported. If you need multiple LLM configurations, please submit a request via a GitHub issue.',
    supplierName: 'Supplier Name',
    remarks: 'Remarks',
    apiKey: 'API Key',
    apiEndpoint: 'API Endpoint',
    apiEndpointHelp: 'Enter the service endpoint URL compatible with OpenAI Response format',
    modelName: 'Model Name',
    placeholder: 'Enter value',
    clear: 'Clear',
    submit: 'Submit',
    submitSuccess: 'Configuration saved',
    submitFailed: 'Save failed, please try again',
    loadFailed: 'Failed to load configuration',
  },

  proxyConfig: {
    cardTitle: 'Proxy Server Settings',
    helpUsage: 'After configuring a proxy server, all network requests will be routed through it. Useful in environments without direct internet access.',
    proxyType: 'Proxy Type',
    typeHttp: 'HTTP Server',
    typeHttps: 'HTTPS Server',
    typeSocks5: 'SOCKS5',
    address: 'Address',
    username: 'Username',
    password: 'Password',
    placeholder: 'Enter value',
    clear: 'Clear',
    submit: 'Submit',
    submitSuccess: 'Configuration saved',
    submitFailed: 'Save failed, please try again',
    loadFailed: 'Failed to load configuration',
  },

  materialConfig: {
    title: 'Material Config',
    pexelsTab: 'Pexels ApiKey',
    pixabayTab: 'Pixabay ApiKey',
    addApiKey: 'Add ApiKey',
    operations: 'Operations',
    delete: 'Delete',
    pexelsColLabel: 'Pexels API Keys',
    pixabayColLabel: 'Pixabay API Keys',
    prevPage: 'Previous',
    nextPage: 'Next',
    deleteConfirm: 'Are you sure you want to delete this API Key?',
    confirmTitle: 'Confirm',
    deleteSuccess: 'Deleted',
    addDialogLabel: '{provider} API Key',
    placeholder: 'Enter value',
    emptyKeyWarning: 'Please enter an API Key',
    cancel: 'Cancel',
    confirm: 'Confirm',
    addSuccess: 'Added',
    addFailed: 'Add failed, please try again',
    deleteFailed: 'Delete failed, please try again',
    loadFailed: 'Failed to load list',
  },

  ttsConfig: {
    title: 'TTS Config',
    usageGuide: 'Usage Guide',
    helpUsage: 'This page lets you configure TTS-related settings, mainly the API Key and similar credentials. Once configured, the corresponding TTS provider will appear when adding a task in "Add Task"; otherwise, only the free "Azure TTS V1" option will be shown.',
    ttsServer: 'TTS Server',
    ttsServerHelp: 'Select the primary engine for voice generation.',
    voiceRole: 'Voice Role',
    testVoice: 'Test Voice Synthesis',
    region: 'Service Region',
    regionPlaceholder: 'Enter (e.g. eastasia)',
    apiKey: 'API Key',
    discard: 'Discard Changes',
    save: 'Save',
    saveSuccess: 'Configuration saved',
    saveFailed: 'Save failed, please try again',
    play: 'Play',
    pause: 'Pause',
    testVoiceFailed: 'Failed to generate voice preview',
    loadListFailed: 'Failed to load TTS service list',
    loadVoiceListFailed: 'Failed to load voice list',
  },
}
