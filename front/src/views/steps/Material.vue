  ...
  <div class="page-wrapper">
    ...
    <div class="table-card">
      <el-table :data="tableData" style="width: 100%"
        ...
        <el-table-column label="素材来源" width="110">
          <template #default="{ row }">
            <el-tag type="info" size="small" effect="light">{{ row.source }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="视频比例" width="100">
          <template #default="{ row }">
            <span class="mono-text">{{ row.aspect }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" align="right">
          <template #default="{ row }">
            <div class="action-btns">
              <el-button size="small" plain type="primary" @click="router.push('/steps/publish')">下一步</el-button>
...
const tableData = computed(() => store.tasks.map(t => ({
  ...
  source: ((t.params?.video_source as string) ?? 'pexels').toUpperCase(),
  aspect: (t.params?.video_aspect as string) ?? '9:16',
  state: t.state,
})))
function stateLabel(s: number) { return s === 1 ? '已搜索' : s === 4 ? '搜索中' : s === -1 ? '失败' : '待处理' }
