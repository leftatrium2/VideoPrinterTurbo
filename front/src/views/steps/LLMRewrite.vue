  ...
  <div class="page-wrapper">
    ...
    <div class="table-card">
      <el-table :data="tableData" style="width: 100%"
        ...
        <el-table-column label="改写指令" min-width="220">
          <template #default="{ row }">
            <span class="clamp-text">{{ row.instruction || '—' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" align="right">
          <template #default="{ row }">
            <div class="action-btns">
              <el-button size="small" plain type="primary" @click="router.push('/steps/tts')">下一步</el-button>
...
const tableData = computed(() => store.tasks.map(t => ({
  ...
  instruction: (t.params?.rewrite_instruction as string) ?? '',
  state: t.state,
})))
function stateLabel(s: number) { return s === 1 ? '已改写' : s === 4 ? '改写中' : s === -1 ? '失败' : '待处理' }
...
.clamp-text { font-size: 13px; color: var(--color-text-regular); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
